...
const reports: Report[] = [
  {
    ...
    result: 'satisfactory',
    issues: [],
    notes: 'Caregiver arrived on time and demonstrated excellent care standards. Child appeared happy and well-cared for.'
  },
  {
    id: 2,
    type: 'quality_issue',
    ...
    notes: 'Observed minor cleanliness issues in elderly care area. Caregiver communication with family needs improvement.'
  },
  {
    id: 3,
    type: 'incident',
    ...
    notes: 'Minor incident - child fell while playing. Caregiver responded appropriately with first aid and contacted parents immediately.'
  }
];
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white pt-12 pb-4 px-4 border-b border-gray-200">
        ...
        <View className="flex-row justify-between">
          <View className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex-1 mr-2">
            <Text className="text-blue-800 text-2xl font-bold">{reports.length}</Text>
          ...
          >
            ...
            <View className="flex-row items-center justify-between mb-3">
              <View className="flex-row items-center">
                <View className="bg-gray-100 w-10 h-10 rounded-xl items-center justify-center mr-3">
                  <Ionicons name={getReportTypeIcon(report.type)} size={20} color="#6b7280" />
                </View>
                <View>
                  <Text className="text-lg font-semibold text-gray-800">
                    {report.type === 'verification' ? 'Verification Report' :
                     report.type === 'quality_issue' ? 'Quality Issue' : 'Incident Report'}
                  </Text>
                  <Text className="text-gray-600 text-sm">{report.assignmentId}</Text>
                </View>
              </View>
              <View className={`px-2 py-1 rounded-full border ${getStatusColor(report.status)}`}>
                <Text className="text-xs font-medium">
                  {report.status === 'under_review' ? 'Under Review' :
                   report.status === 'submitted' ? 'Submitted' : 'Resolved'}
                </Text>
              </View>
            </View>
            ...
            <View className="mb-3">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-gray-600 ml-2">{report.family} • {report.caregiver}</Text>
              </View>
              <View className="flex-row items-center mb-2">
            ...
            <View className="flex-row justify-between items-center">
              <View className={`px-2 py-1 rounded-full border ${getResultColor(report.result)}`}>
                <Text className="text-xs font-medium">
                  {report.result === 'satisfactory' ? 'Satisfactory' :
                   report.result === 'requires_attention' ? 'Needs Attention' : 'Incident Handled'}
                </Text>
              </View>
              <TouchableOpacity>
            ...
            <ScrollView className="flex-1 px-4 py-4">
              <View className="bg-gray-50 rounded-2xl p-4 mb-4">
                <Text className="text-lg font-semibold text-gray-800 mb-2">
                  {selectedReport.type === 'verification' ? 'Verification Report' :
                   selectedReport.type === 'quality_issue' ? 'Quality Issue Report' : 'Incident Report'}
                </Text>
                <Text className="text-gray-600 mb-1">ID: {selectedReport.assignmentId}</Text>
                <Text className="text-gray-600 mb-1">Family: {selectedReport.family}</Text>
                <Text className="text-gray-600">Caregiver: {selectedReport.caregiver}</Text>
              </View>

              <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-200">
                <Text className="text-base font-semibold text-gray-800 mb-3">Report Notes</Text>
                <Text className="text-gray-600 leading-5">{selectedReport.notes}</Text>
              </View>

              {selectedReport.issues.length > 0 && (
                <View className="bg-yellow-50 rounded-2xl p-4 mb-4 border border-yellow-200">
                  <Text className="text-base font-semibold text-yellow-800 mb-3">Issues Identified</Text>
                  <View className="flex-row flex-wrap">
                    {selectedReport.issues.map((issue, index) => (
                      <View key={index} className="bg-yellow-100 px-3 py-1 rounded-full mr-2 mb-2">
              ...
              <TouchableOpacity className="bg-blue-500 rounded-xl py-4 items-center mt-4">
                <Text className="text-white font-semibold">Download PDF Report</Text>
              </TouchableOpacity>
            </ScrollView>
          )}
        </View>
      ...
      >
        <View className="flex-1 bg-white">
          <View className="pt-12 pb-4 px-4 border-b border-gray-200">
            <View className="flex-row items-center justify-between">
              <Text className="text-xl font-bold text-gray-800">Create New Report</Text>
          ...
          <ScrollView className="flex-1 px-4 py-4">
            <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-200">
              <Text className="text-base font-semibold text-gray-800 mb-3">Report Type</Text>
              <View className="space-y-2">
                {[
                  { value: 'verification', label: 'Verification Report', icon: 'checkmark-circle-outline' },
                  { value: 'quality_issue', label: 'Quality Issue Report', icon: 'warning-outline' },
                  { value: 'incident', label: 'Incident Report', icon: 'alert-circle-outline' }
                ].map((type) => (
                  <TouchableOpacity
                    key={type.value}
                  ...
                  >
                    <Ionicons
                      ...
                      size={24}
                      color={newReportType === type.value ? '#3b82f6' : '#6b7280'}
                    />
                    <Text className={`ml-3 font-medium ${
                      newReportType === type.value ? 'text-blue-800' : 'text-gray-800'
                    }`}>
                      {type.label}
                    </Text>
                  </TouchableOpacity>
            ...
            <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-200">
              <Text className="text-base font-semibold text-gray-800 mb-3">Assignment</Text>
              <View className="bg-gray-50 border border-gray-300 rounded-xl p-3">
                ...
                <Text className="text-gray-600 text-sm">The Jayawardene Family • David Lee</Text>
              </View>
            </View>

            <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-200">
              <Text className="text-base font-semibold text-gray-800 mb-3">Report Details</Text>
              <TextInput
