    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Start',
          onPress: () => {
            Alert.alert('Assignment Started', 'GPS tracking activated. Good luck!');
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            Alert.alert('Location Logged', 'Your arrival has been recorded with GPS coordinates.');
          }
        }
      ]
    );
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white pt-12 pb-4 px-4 border-b border-gray-200">
        <View className="flex-row items-center justify-between mb-4">
          <Text className="text-2xl font-bold text-gray-800">My Assignments</Text>
          <View className="bg-blue-100 px-3 py-1 rounded-full">
            <Text className="text-blue-700 text-sm font-medium">4/5 Capacity</Text>
          </View>
        </View>
        ...
        {filteredAssignments.map((assignment) => (
          ...
          >
            ...
            <View className="flex-row items-center justify-between mb-3">
              <Text className="text-lg font-semibold text-gray-800">{assignment.family}</Text>
              <View className={`px-2 py-1 rounded-full border ${getStatusColor(assignment.status)}`}>
            ...
            <View className="mb-3">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-gray-600 ml-2">Caregiver: {assignment.caregiver}</Text>
              </View>
              <View className="flex-row items-center mb-2">
                <Ionicons name="location-outline" size={16} color="#6b7280" />
                <Text className="text-gray-600 ml-2 flex-1">{assignment.address}</Text>
              </View>
              <View className="flex-row items-center mb-2">
                <Ionicons name="time-outline" size={16} color="#6b7280" />
                <Text className="text-gray-600 ml-2">{formatTime(assignment.scheduledTime)}</Text>
                <Text className="text-gray-500 ml-2">• {assignment.estimatedDuration} min</Text>
              </View>
            </View>

            {/* Priority and Service Type */}
            <View className="flex-row items-center justify-between">
              <View className={`px-2 py-1 rounded-full border ${getPriorityColor(assignment.priority)}`}>
                <Text className="text-xs font-medium">
                  {assignment.priority.charAt(0).toUpperCase() + assignment.priority.slice(1)} Priority
                </Text>
              </View>
              <Text className="text-sm text-gray-500">{assignment.serviceType}</Text>
            </View>
            ...
            <View className="flex-row justify-end mt-3 pt-3 border-t border-gray-100">
              {assignment.status === 'scheduled' && (
                <>
                  ...
                  >
                    <Text className="text-blue-600 text-sm font-medium">Navigate</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
              ...
              {assignment.status === 'in-progress' && (
                <TouchableOpacity className="bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2">
                  <Text className="text-yellow-600 text-sm font-medium">Complete</Text>
                </TouchableOpacity>
              )}
              {assignment.status === 'completed' && (
                <TouchableOpacity className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                  <Text className="text-gray-600 text-sm font-medium">View Report</Text>
                </TouchableOpacity>
              )}
            </View>
          </TouchableOpacity>
      ...
      >
        <View className="flex-1 bg-white">
          <View className="pt-12 pb-4 px-4 border-b border-gray-200">
            <View className="flex-row items-center justify-between">
              <Text className="text-xl font-bold text-gray-800">Assignment Details</Text>
          ...
          {selectedAssignment && (
            <ScrollView className="flex-1 px-4 py-4">
              <View className="bg-gray-50 rounded-2xl p-4 mb-4">
                <Text className="text-lg font-semibold text-gray-800 mb-2">{selectedAssignment.family}</Text>
                <Text className="text-gray-600 mb-1">{selectedAssignment.assignmentNumber}</Text>
                <Text className="text-gray-600">{selectedAssignment.address}</Text>
              </View>

              <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-200">
                <Text className="text-base font-semibold text-gray-800 mb-3">Service Details</Text>
                <Text className="text-gray-600 mb-2">Service Type: {selectedAssignment.serviceType}</Text>
                <Text className="text-gray-600 mb-2">Caregiver: {selectedAssignment.caregiver}</Text>
                <Text className="text-gray-600 mb-2">Duration: {selectedAssignment.estimatedDuration} minutes</Text>
                <Text className="text-gray-600">Time: {formatTime(selectedAssignment.scheduledTime)}</Text>
              </View>

              <View className="bg-white rounded-2xl p-4 mb-4 border border-gray-200">
                <Text className="text-base font-semibold text-gray-800 mb-3">Notes</Text>
                <Text className="text-gray-600 leading-5">{selectedAssignment.notes}</Text>
              </View>

              {selectedAssignment.verificationResult && (
                <View className="bg-green-50 rounded-2xl p-4 border border-green-200">
                  <Text className="text-base font-semibold text-green-800 mb-2">Verification Result</Text>
                  <Text className="text-green-700">{selectedAssignment.verificationResult}</Text>
      ...
      >
        <View className="flex-1 bg-white">
          <View className="pt-12 pb-4 px-4 border-b border-gray-200">
            <View className="flex-row items-center justify-between">
              <Text className="text-xl font-bold text-gray-800">Location Tracking</Text>
            ...
            <View className="mt-6">
              ...
              <View className="space-y-3">
                ...
                >
                  ...
                  <Text className="text-white font-semibold ml-2">Mark Arrived</Text>
                </TouchableOpacity>
                ...
                >
                  ...
                  <Text className="text-white font-semibold ml-2">Call Family</Text>
                </TouchableOpacity>

                <CameraCapture
                  onPhotoTaken={(photoUri) => {
                    console.log('Photo captured:', photoUri);
                    Alert.alert('Photo Saved', 'Evidence photo has been attached to this assignment.');
                  }}
                />
              </View>
            </View>
      ...
      {selectedAssignment && (
        ...
        >
          <View className="flex-1 bg-white">
            <View className="pt-12 pb-4 px-4 border-b border-gray-200">
              <View className="flex-row items-center justify-between">
                <Text className="text-xl font-bold text-gray-800">Field Documentation</Text>
              ...
              <View className="space-y-4">
                <CameraCapture
                  onPhotoTaken={(photoUri) => {
                    ...
                    Alert.alert('Evidence Captured', 'Photo has been saved to assignment documentation.');
                  }}
                />
                ...
                <TouchableOpacity className="bg-yellow-500 rounded-xl py-4 flex-row items-center justify-center">
                  ...
                  <Text className="text-white font-semibold ml-2">Report Issue</Text>
                </TouchableOpacity>

                <TouchableOpacity className="bg-green-500 rounded-xl py-4 flex-row items-center justify-center">
                  <Ionicons name="checkmark-circle-outline" size={24} color="#ffffff" />
                  <Text className="text-white font-semibold ml-2">Complete Assignment</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
