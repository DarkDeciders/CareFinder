...
const assignments = [
  ...
  },
  {
    ...
    coordinates: { lat: 6.9270, lng: 79.8612 },
    time: '16:30',
    status: 'scheduled',
    distance: 3.2,
    estimatedTravel: 10
...
export default function NavigationScreen() {
  ...
  const optimizeRoute = () => {
    ...
    Alert.alert('Route Optimized', 'Your route has been optimized for minimal travel time.');
  };

  const totalDistance = assignments.reduce((sum, assignment) => sum + assignment.distance, 0);
  const totalTime = assignments.reduce((sum, assignment) => sum + assignment.estimatedTravel, 0);
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <View className="bg-white pt-12 pb-4 px-4 border-b border-gray-200">
        <Text className="text-2xl font-bold text-gray-800 mb-4">Route Navigation</Text>

        {/* Route Summary */}
        <View className="bg-blue-50 border border-blue-200 rounded-2xl p-4 mb-4">
          <View className="flex-row items-center justify-between mb-2">
            <Text className="text-blue-800 font-semibold">Today&apos;s Route</Text>
            <TouchableOpacity
              onPress={optimizeRoute}
              className={`px-3 py-1 rounded-full ${routeOptimized ? 'bg-green-100 border-green-300' : 'bg-blue-100 border-blue-300'} border`}
            >
              <Text className={`text-xs font-medium ${routeOptimized ? 'text-green-700' : 'text-blue-700'}`}>
          ...
          <View className="flex-row justify-between">
            <View className="flex-1">
              ...
              <Text className="text-blue-800 font-semibold">{totalDistance.toFixed(1)} km</Text>
            </View>
            <View className="flex-1">
              <Text className="text-blue-600 text-sm">Est. Travel Time</Text>
              <Text className="text-blue-800 font-semibold">{totalTime} minutes</Text>
            </View>
            <View className="flex-1">
              <Text className="text-blue-600 text-sm">Assignments</Text>
              <Text className="text-blue-800 font-semibold">{assignments.length} locations</Text>
            </View>
          </View>
        </View>
      </View>
      ...
      <ScrollView className="flex-1 px-4 py-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white rounded-2xl p-4 mb-4 shadow-sm border border-gray-100">
          <View className="flex-row items-center mb-2">
            ...
            <Text className="text-green-700 font-semibold">Current Location</Text>
          </View>
          <Text className="text-gray-600 ml-7">CareFinder Office, Colombo 02</Text>
        ...
        {assignments.map((assignment, index) => (
          <View key={assignment.id} className="relative">
            ...
            <TouchableOpacity
              onPress={() => setSelectedAssignment(assignment.id === selectedAssignment ? null : assignment.id)}
            ...
            >
              ...
              <View className="flex-row items-center justify-between mb-3">
                <View className="flex-row items-center">
                  <View className={`w-8 h-8 rounded-full items-center justify-center mr-3 ${
                    assignment.status === 'next' ? 'bg-blue-500' : 'bg-gray-300'
                  }`}>
                    <Text className={`text-sm font-bold ${
                      assignment.status === 'next' ? 'text-white' : 'text-gray-600'
                    }`}>
                      {index + 1}
                    </Text>
                  </View>
                  <View>
                    ...
                    {assignment.status === 'next' && (
                      <Text className="text-blue-600 text-sm font-medium">NEXT STOP</Text>
                    )}
                  </View>
                </View>
                <Text className="text-gray-500 font-medium">{assignment.time}</Text>
              </View>
              ...
              <View className="flex-row items-center justify-between ml-11">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-gray-600 ml-2">{assignment.distance} km • {assignment.estimatedTravel} min</Text>
                </View>
                <TouchableOpacity
                ...
                <View className="mt-4 pt-4 border-t border-gray-100 ml-11">
                  <View className="bg-gray-50 rounded-xl p-3 mb-3">
                    <Text className="text-gray-700 text-sm">
                      Coordinates: {assignment.coordinates.lat.toFixed(4)}, {assignment.coordinates.lng.toFixed(4)}
                    </Text>
                  </View>

                  <View className="flex-row justify-between">
                    <TouchableOpacity className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 flex-1 mr-2">
                      <Text className="text-green-600 text-sm font-medium text-center">Call Family</Text>
                    </TouchableOpacity>
                    <TouchableOpacity className="bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2 flex-1 ml-2">
                      <Text className="text-yellow-600 text-sm font-medium text-center">Mark Arrived</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              )}
        ...
        <View className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <View className="flex-row items-center mb-2">
            ...
            <Text className="text-gray-700 font-semibold">Return to Office</Text>
          </View>
          <Text className="text-gray-600 ml-7">CareFinder Office, Colombo 02</Text>
        ...
        <View className="bg-red-50 border border-red-200 rounded-2xl p-4 mt-4 mb-8">
          <View className="flex-row items-center mb-2">
            ...
            <Text className="text-red-800 font-semibold ml-2">Emergency Support</Text>
          </View>
          <Text className="text-red-700 text-sm mb-3">For urgent issues during field visits</Text>
          <TouchableOpacity className="bg-red-500 rounded-lg py-3 items-center">
            <Text className="text-white font-semibold">Call Emergency Line</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </View>
