  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Training Schedule</h1>
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50">
        ...
        <div className="hidden lg:block">
          <div className="grid grid-cols-8 gap-2">
            ...
            {days.map((day) => (
              <div key={day} className="p-3 text-center font-semibold text-gray-800 dark:text-gray-200 text-sm bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                {day.substring(0, 3)}
              </div>
            ))}
              ...
              <React.Fragment key={time}>
                ...
                {days.map((day, dayIndex) => {
                  const session = sessions.find(s => {
                    ...
                    return sessionDate.getDay() === (dayIndex + 1) % 7 && s.time === time;
                  });
                  ...
                  return (
                    <div key={`${day}-${time}`} className="p-1 min-h-[80px] border-2 border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      {session && (
                        <div className={`p-2 rounded-lg text-xs font-medium border-l-4 ${
                          ...
                          <div className="font-semibold text-xs">{session.trainee}</div>
                          <div className="text-xs mt-1 opacity-90">{session.type}</div>
                          <div className="text-xs mt-1 opacity-75">{session.duration}min</div>
                        </div>
                      )}
                    </div>
                  );
                  ...
                  {daySessions.length > 0 ? (
                    <div className="space-y-2">
                      {daySessions.map(session => (
                        <div key={session.id} className={`p-3 rounded-lg border-l-4 ${
                          session.status === 'assessment'
                            ? 'bg-warning-50 text-warning-900 border-warning-400 dark:bg-warning-900/30 dark:text-warning-200 dark:border-warning-500'
                            : 'bg-primary-50 text-primary-900 border-primary-400 dark:bg-primary-900/30 dark:text-primary-200 dark:border-primary-500'
                        }`}>
                          <div className="font-medium text-sm">{session.trainee}</div>
                          <div className="text-xs mt-1">{session.type}</div>
                          <div className="text-xs mt-1 font-medium">{session.time} • {session.duration}min</div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <p className="text-sm text-gray-500 dark:text-gray-400 italic">No sessions scheduled</p>
                  )}
                </div>
              );
            })}
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-700/50">
        <h2 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-4">Upcoming Sessions</h2>
        <div className="space-y-3 sm:space-y-4">
          {sessions.map((session) => (
            <div key={session.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-gray-50/80 dark:bg-gray-700/80 rounded-lg border border-gray-200 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
              <div className="flex items-start sm:items-center space-x-4">
                <div className={`w-4 h-4 rounded-full mt-0.5 sm:mt-0 flex-shrink-0 ${
                  session.status === 'assessment' ? 'bg-warning-500' : 'bg-primary-500'
                }`}></div>
                <div className="min-w-0">
                  <h3 className="font-semibold text-gray-900 dark:text-white text-sm sm:text-base">{session.trainee}</h3>
                  <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 font-medium">{session.type}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-700/50 text-center hover:bg-white/90 dark:hover:bg-gray-800/90 transition-colors cursor-pointer">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white mb-2 text-sm sm:text-base">Add Session</h3>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Schedule a new training session</p>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-700/50 text-center hover:bg-white/90 dark:hover:bg-gray-800/90 transition-colors cursor-pointer">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white mb-2 text-sm sm:text-base">Assessments</h3>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Schedule final assessments</p>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-700/50 text-center hover:bg-white/90 dark:hover:bg-gray-800/90 transition-colors cursor-pointer">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white mb-2 text-sm sm:text-base">Availability</h3>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">Manage your availability</p>
        </div>
      </div>
    </div>
  );
