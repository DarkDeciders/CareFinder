...
const CAREFINDER_KNOWLEDGE = {
  services: {
    childcare: 'We provide professional childcare services with certified caregivers for children of all ages.',
    elderlycare: 'Our elderly care services include companionship, medication reminders, meal preparation, and mobility assistance.',
    specialneeds: 'Specialized care for individuals with special needs, including autism support, disability care, and therapeutic assistance.',
    emergency: 'Emergency care services available 24/7 for urgent care needs.',
  },
  process: {
    booking: 'To book a caregiver: 1) Create your profile, 2) Search for caregivers in your area, 3) Review profiles and ratings, 4) Message caregivers, 5) Book and pay securely.',
    verification: 'All caregivers undergo background checks, document verification, and on-site quality inspections by our agents.',
    training: 'Unqualified caregivers receive 3-month training programs at our facilities before being approved.',
    payment: 'Secure payments through PayHere with escrow protection - funds are released after service completion.',
  },
  platform: {
    roles: 'CareFinder supports 5 user types: Families (care seekers), Caregivers, Trainers, Quality Agents, and Administrators.',
    quality: 'Our agents conduct on-site visits to verify service quality and ensure caregiver performance meets standards.',
    support: 'Secure messaging, real-time notifications, and 24/7 customer support available.',
  },
  location: 'CareFinder operates across Sri Lanka with location-based matching and agent coverage.',
};
...
export function useChatbot() {
  ...
  const generateResponse = useCallback((userMessage: string): string => {
    ...
    if (message.includes('childcare') || message.includes('child') || message.includes('kids')) {
    ...
    if (message.includes('elderly') || message.includes('senior') || message.includes('old')) {
    ...
    if (message.includes('special needs') || message.includes('disability') || message.includes('autism')) {
    ...
    if (message.includes('how to book') || message.includes('booking') || message.includes('hire')) {
    ...
    if (message.includes('verification') || message.includes('background') || message.includes('qualified')) {
    ...
    if (message.includes('agent') || message.includes('quality') || message.includes('inspection')) {
    ...
    // Location queries
    if (message.includes('near me') || message.includes('location') || message.includes('area')) {
