...
const scheduledEvents: Record<string, ScheduledEvent[]> = {
  "2025-09-19": [
    { time: "10:00 AM", family: "The Perera Family", service: "Childcare" },
  ],
  "2025-09-20": [
    { time: "9:00 AM", family: "Mr. Silva", service: "Elder Care" },
  ],
  "2025-09-22": [
    { time: "4:00 PM", family: "The De Silva Family", service: "Tutoring" },
  ],
  "2025-09-25": [
    { time: "3:00 PM", family: "The Jayasuriya Family", service: "Childcare" },
  ],
};
  ...
  const renderCalendarDays = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        ...
        >
          <span
            className={`font-medium text-xs sm:text-sm ${isToday ? "text-primary-600 dark:text-primary-400" : "text-gray-800 dark:text-gray-200"}`}
          >
            {day}
          </span>
    ...
    <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200/50 dark:border-gray-700/50">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 space-y-2 sm:space-y-0">
        ...
        <div className="flex items-center space-x-3">
          <span className="font-semibold text-base sm:text-lg text-primary-600 dark:text-primary-400">
            {monthName} {year}
          </span>
        </div>
      </div>
        ...
        <ul className="space-y-2 sm:space-y-3">
          {Object.entries(scheduledEvents).flatMap(([date, events]) =>
            events.map((event, index) => (
              <li
                key={`${date}-${index}`}
                className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg flex items-start sm:items-center space-x-3 sm:space-x-4 hover:shadow-sm transition-shadow"
              >
