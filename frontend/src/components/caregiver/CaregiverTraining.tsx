...
export default function CaregiverTraining() {
  ...
  const modules = [
    ...
    {
      name: "Child Safety Protocols",
      completed: trainingStatus === "qualified",
      score: trainingStatus === "qualified" ? 89 : null,
      date: trainingStatus === "qualified" ? "2024-10-30" : null,
    },
    {
      name: "Emergency Procedures",
      completed: trainingStatus === "qualified",
      score: trainingStatus === "qualified" ? 96 : null,
      date: trainingStatus === "qualified" ? "2024-11-05" : null,
    },
    {
      name: "Final Assessment",
      completed: trainingStatus === "qualified",
      score: trainingStatus === "qualified" ? 94 : null,
      date: trainingStatus === "qualified" ? "2024-11-10" : null,
    },
  ];
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              You have successfully completed the CareFinder training program
              and are certified to provide care services.
            </p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-success-600 dark:text-success-400">
              {trainingData.qualified.overallScore}%
            </div>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {new Date(
                trainingData.qualified.completionDate,
              ).toLocaleDateString()}
            </p>
          </div>
          <div>
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {trainingData.qualified.certificateNumber}
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {trainingData.qualified.trainer}
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="font-medium text-gray-900 dark:text-white">
                CareFinder Training Certificate
              </h4>
      ...
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {new Date(
                trainingData.in_training.startDate,
              ).toLocaleDateString()}
            </p>
          </div>
          <div>
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {trainingData.in_training.trainer}
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
            ...
            <p className="text-sm text-gray-900 dark:text-white">
              {new Date(
                trainingData.in_training.nextSession,
              ).toLocaleDateString()}{" "}
              at{" "}
              {new Date(
                trainingData.in_training.nextSession,
    ...
    <div className="space-y-6">
      <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4">
          Your application is being reviewed and a trainer will be assigned
          soon.
        </p>
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
          <p className="text-yellow-800 dark:text-yellow-400 text-sm">
            <strong>Expected Start:</strong>{" "}
            {trainingData.pending.expectedStart}
          </p>
          <p className="text-yellow-700 dark:text-yellow-300 text-xs mt-1">
            Applied on:{" "}
            {new Date(
              trainingData.pending.applicationDate,
            ).toLocaleDateString()}
          </p>
        </div>
      </div>
        ...
        <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50">
          ...
          <div className="space-y-4">
            {modules.map((module, index) => (
              <div
                key={index}
                className="flex items-center justify-between p-4 bg-gray-50/50 dark:bg-gray-700/50 rounded-lg"
              >
                <div className="flex items-center space-x-3">
                  <div
